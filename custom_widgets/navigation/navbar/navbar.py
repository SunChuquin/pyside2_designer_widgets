...
from PySide2.QtGui import QColor, QResizeEvent, QMouseEvent, QKeyEvent, QPaintEvent, QPainter, QLinearGradient, QPen, QFont
from PySide2.QtCore import Slot, Signal, QRectF, QTimer, QSize, QPointF, QEnum, QObject, Qt, QPoint
...
class NavBar(QWidget):
    ...
    def drawText(self, painter):  # drawText(self, painter: PySide2.QtGui.QPainter)
        ...
        count = len(self.__listItem)  # int
        textLen = Decimal(0)  # decimal.Decimal
        strText = ""  # str
        self.__initLen = 0
    ...
    bgColorStart = property(fget=getBgColorStart, fset=setBgColorStart, fdel=None, doc="导航条主背景渐变开始颜色")
    bgColorEnd = property(fget=getBgColorEnd, fset=setBgColorEnd, fdel=None, doc="导航条主背景渐变结束颜色")

    barColorStart = property(fget=getBarColorStart, fset=setBarColorStart, fdel=None, doc="导航条当前条目渐变开始颜色")
    barColorEnd = property(fget=getBarColorEnd, fset=setBarColorEnd, fdel=None, doc="导航条当前条目渐变结束颜色")

    textNormalColor = property(fget=getTextNormalColor, fset=setTextNormalColor, fdel=None, doc="文字正常颜色")
    textSelectColor = property(fget=getTextSelectColor, fset=setTextSelectColor, fdel=None, doc="文字选中颜色")

    items = property(fget=getItems, fset=setItems, fdel=None, doc="所有条目文字信息")
    currentIndex = property(fget=getCurrentIndex, fset=setCurrentIndex, fdel=None, doc="当前选中条目索引")
    currentItem = property(fget=getCurrentItem, fset=setCurrentItem, fdel=None, doc="当前选中条目文字")

    bgRadius = property(fget=getBgRadius, fset=setBgRadius, fdel=None, doc="背景圆角半径")
    barRadius = property(fget=getBarRadius, fset=setBarRadius, fdel=None, doc="选中条目背景圆角半径")
    space = property(fget=getSpace, fset=setSpace, fdel=None, doc="条目元素之间的间距")

    lineWidth = property(fget=getLineWidth, fset=setLineWidth, fdel=None, doc="线条宽度")
    lineColor = property(fget=getLineColor, fset=setLineColor, fdel=None, doc="线条颜色")

    barStyle = property(fget=getBarStyle, fset=setBarStyle, fdel=None, doc="选中元素样式")

    keyMove = property(fget=getKeyMove, fset=setKeyMove, fdel=None, doc="是否支持按键移动")
    horizontal = property(fget=getHorizontal, fset=setHorizontal, fdel=None, doc="是否横向显示")
    flat = property(fget=getFlat, fset=setFlat, fdel=None, doc="是否扁平化")
